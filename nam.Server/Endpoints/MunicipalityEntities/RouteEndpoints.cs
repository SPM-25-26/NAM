...
namespace nam.Server.Endpoints.MunicipalityEntities
{
    public class RouteEndpoints
    {
        ...
        public static async Task<IResult> GetCardList(
            [FromServices] IMunicipalityEntityService<RouteCard, RouteDetail> routeService,
            ...
            )
        {
            try
            {
                var result = await routeService.GetCardListAsync(municipality, language);
            ...
            )
        {
            try
            {
                var result = await routeService.GetCardDetailAsync(identifier, language);
                return TypedResults.Ok(result);
            }
            catch (Exception ex)
            {
            ...
            )
        {
            try
            {
                var result = await routeService.GetFullCardAsync(identifier, language);
                return TypedResults.Ok(result);
            }
            catch (Exception ex)
            {
         ...
         )
        {
            try
            {
                var result = await routeService.GetFullCardListAsync(municipality, language);
                return TypedResults.Ok(result);
            }
            catch (Exception ex)
            {
