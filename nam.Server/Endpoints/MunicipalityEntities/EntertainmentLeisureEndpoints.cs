...
namespace nam.Server.Endpoints.MunicipalityEntities
{
    public class EntertainmentLeisureEndpoints
    {
           ...
           )
        {
            try
            {
                var result = await entertainmentLeisureService.GetFullCardAsync(identifier, language);
                return TypedResults.Ok(result);
            }
            catch (Exception ex)
            {
          ...
          )
        {
            try
            {
                var result = await entertainmentLeisureService.GetFullCardListAsync(municipality, language);
                return TypedResults.Ok(result);
            }
            catch (Exception ex)
            {
