...
{
    [TestFixture]
    public class PublicEventRepositoryTests
    {
        private static DbContextOptions<ApplicationDbContext> CreateOptions()
        {
            var databaseName = $"PublicEventRepositoryTests_{NUnit.Framework.TestContext.CurrentContext.Test.Name}";
            return new DbContextOptionsBuilder<ApplicationDbContext>()
                .UseInMemoryDatabase(databaseName)
                .Options;
        }

        private static PublicEventMobileDetail CreateDetail(Guid entityId, string municipalityName = "Milano")
        {
        ...
        private static PublicEventCard CreateCard(Guid entityId, string municipalityName = "Milano")
        {
            return new PublicEventCard
            {
                EntityId = entityId,
                EntityName = "Event",
                ImagePath = "image.png",
        ...
        public async Task GetByEntityIdAsync_ReturnsMatchingCard()
        {
            ...
            var entityId = Guid.Parse("30303030-3030-3030-3030-303030303031");
            var cancellationToken = CancellationToken.None;

            await using (var setupContext = new ApplicationDbContext(options))
            {
                setupContext.PublicEventCards.Add(CreateCard(entityId));
                await setupContext.SaveChangesAsync(cancellationToken);
            }

            await using var context = new ApplicationDbContext(options);
            var repository = new PublicEventRepository(context);
        ...
        {
            var options = CreateOptions();
            var milanoId = Guid.Parse("32323232-3232-3232-3232-323232323232");
            var romaId = Guid.Parse("33333333-3333-3333-3333-333333333335");
            var cancellationToken = CancellationToken.None;

            await using (var setupContext = new ApplicationDbContext(options))
            {
                setupContext.PublicEventCards.Add(CreateCard(milanoId, "Milano"));
                setupContext.PublicEventCards.Add(CreateCard(romaId, "Roma"));
                await setupContext.SaveChangesAsync(cancellationToken);
            }
        ...
        {
            var options = CreateOptions();
            var entityId = Guid.Parse("34343434-3434-3434-3434-343434343434");
            var cancellationToken = CancellationToken.None;

            await using (var setupContext = new ApplicationDbContext(options))
            {
                setupContext.PublicEventMobileDetails.Add(CreateDetail(entityId));
                await setupContext.SaveChangesAsync(cancellationToken);
            }
        ...
        {
            var options = CreateOptions();
            var entityId = Guid.Parse("35353535-3535-3535-3535-353535353535");
            var cancellationToken = CancellationToken.None;
        ...
        {
            var options = CreateOptions();
            var entityId = Guid.Parse("36363636-3636-3636-3636-363636363636");
            var cancellationToken = CancellationToken.None;

            await using (var setupContext = new ApplicationDbContext(options))
            {
                setupContext.PublicEventCards.Add(CreateCard(entityId, "Milano"));
                await setupContext.SaveChangesAsync(cancellationToken);
            }
        ...
        {
            var options = CreateOptions();
            var entityId = Guid.Parse("37373737-3737-3737-3737-373737373737");
            var cancellationToken = CancellationToken.None;

            await using (var setupContext = new ApplicationDbContext(options))
            {
                setupContext.PublicEventCards.Add(new PublicEventCard
                {
                    EntityId = entityId,
                    EntityName = "Event",
                    ImagePath = "image.png",
        ...
        {
            var options = CreateOptions();
            var entityId = Guid.Parse("38383838-3838-3838-3838-383838383838");
            var cancellationToken = CancellationToken.None;
