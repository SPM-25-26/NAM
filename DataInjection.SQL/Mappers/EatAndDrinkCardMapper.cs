...
{
    public class EatAndDrinkCardMapper : IDtoMapper<List<EatAndDrinkCardDto>, List<EatAndDrinkCard>>
    {
        public List<EatAndDrinkCard> MapToEntity(List<EatAndDrinkCardDto> dtos)
        {
            if (dtos == null || dtos.Count == 0)
                return [];

            return dtos.Where(dto => dto is not null)
                .Select(dto => new EatAndDrinkCard
                {
